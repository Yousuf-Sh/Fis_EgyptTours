...
@include('Admin.include.asidebar')
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>FAQs
      <a type="submit" class="btn btn-danger btn-sm" style="float:right;" href="{{url('admin/faq')}}">Back</a>
      </h1>
      <nav>
        <ol class="breadcrumb">
                        ...
                        <div class="tab-content" id="languageTabsContent">
                            @foreach($languages as $index => $language)
                                @php
                                    ...
                                    $question = $questions->where('language', $language->slug)->first();
                                ...
                                <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" 
                                    ...
                                    <form class="row g-3 my-3" method="POST" action="{{ route('faq.update', $primaryQuestion->id) }}" enctype="multipart/form-data">
                                        @csrf
                                        @method('POST')
                                        <div class="col-md-6">
                                            <label for="title_{{ $language->slug }}" class="form-label">Question</label>
                                            <input type="text" 
                                                   name="{{ $language->slug }}_title" 
                                                   class="form-control" 
                                                   id="title_{{ $language->slug }}" 
                                                   value="{{ old("{$language->slug}_title", $question->question ?? ($language->slug === 'en' ? $primaryQuestion->question : '')) }}" 
                                                   {{ $language->slug === 'ar' ? 'style=direction:rtl;' : '' }} 
                                                   required>
                                            <input type="hidden" name="language" value="{{ $language->slug }}">
                                        </div>
                                        <div class="col-md-12">
                                            <label for="description_{{ $language->slug }}" class="form-label">Answer</label>
                                            <textarea name="{{ $language->slug }}_description" 
                                                      class="form-control ckeditor" 
                                                      id="description_{{ $language->slug }}" 
                                                      rows="10" 
                                                      {{ $language->slug === 'ar' ? 'style=direction:rtl;' : '' }} 
                                                      required>{{ old("{$language->slug}_description", $question->answer ?? ($language->slug === 'en' ? $primaryQuestion->answer : '')) }}</textarea>
                                        </div>
                                       
                                        @if ($language->slug !== 'en')
                                            <input type="hidden" name="secondary_id" value="{{ $primaryQuestion->id }}">
                                        @endif
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <img src="{{ Storage::url($primaryQuestion->image) }}" id="output1" width="100" class="my-3">
