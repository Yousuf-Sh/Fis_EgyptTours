            ...
            <div class="tab-content" id="languageTabsContent">
              @foreach($languages as $index => $language)
                  ...
                  <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" 
                      ...
                      <form class="row g-3 my-3" method="POST" action="{{ route('slider.store') }}" enctype="multipart/form-data">
                          @csrf
                          <div class="col-md-6">
                          ...
                          <div class="col-md-12">
                              ...
                              <textarea name="{{ $language->slug }}_description" 
                                ...
                                rows="10" {{ $language->slug === 'ar' ? 'style=direction:rtl;' : '' }} required>
                                {{ old("{$language->slug}_description", $slider->description ?? ($language->slug === 'en' ? $PrimarySlider->description : '')) }}
                              </textarea>
                          </div>
                      </form> 
                  </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // Restore the video preview function
    function loadFile(event, outputId) {
        const input = event.target;
        const outputElement = document.getElementById(outputId);
    ...
    // Add event listeners to all file inputs for previewing videos
    const fileInputs = document.querySelectorAll('input[type="file"]');
    fileInputs.forEach((input) => {
        // Dynamically set output ID based on input ID
        const outputId = input.id.replace('vidInp', 'output');
    ...
    submitButton.addEventListener('click', function (event) {
        ...
        try {
            ...
            // Specifically handle file inputs for videos
            const videoFileInputs = document.querySelectorAll('input[name^="videos["]');
            
            console.log('File Inputs Found:', videoFileInputs.length);
            ...
            // Determine submission URL (assuming Laravel route)
            const currentUrl = "{{ route('slider.store')}}";
