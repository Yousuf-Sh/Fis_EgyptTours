...
<script>
 document.addEventListener('DOMContentLoaded', function () {
    function setupButtonFieldToggle(form) {
        console.log('Setting up button field toggle for form:', form);

        const addButtonLink = form.querySelector('.add-button-link');
        const buttonFieldsId = form.querySelector('.buttonFields');
        console.log(buttonFieldsId); // Select by ID prefix
        const removeButtonLink = buttonFieldsId ? buttonFieldsId.querySelector('.remove-button-link') : null;

        if (!addButtonLink || !buttonFieldsId || !removeButtonLink) {
            console.error('Error: Missing elements in form', form);
            return;
        }

        addButtonLink.addEventListener('click', function (event) {
            event.preventDefault();
            buttonFieldsId.style.display = 'block';
            addButtonLink.style.display = 'none';
        });

        removeButtonLink.addEventListener('click', function (event) {
            event.preventDefault();
            buttonFieldsId.style.display = 'none';
            addButtonLink.style.display = 'inline-block';
            buttonFieldsId.querySelectorAll('input').forEach(input => (input.value = ''));
        });

        console.log('Button field toggle setup complete for form:', form);
    }

    const forms = document.querySelectorAll('form');
    forms.forEach(setupButtonFieldToggle);
});
